...
class Detail extends Component {	
    ...
    CellFormatter(cell, row) {
        return (<div><Link to={`/table5C/provinsi?provinsi=${row.provinsi}`}>{row.provinsi}</Link></div>);
    }
    ...
    render() {
        ...
        return (
            ...
            namaLaporan = {`YANKB Laporan Tempat Pelayanan KB Swasta Berdasarkan Status Kerjasama Dengan BPJS Kesehatan`}
            marginLeft = {200}
            header = {
                [
                    [
