...
class Detail extends Component {	
    ...
    CellFormatter(cell, row) {
        return (<div><Link to={`/table8B/provinsi?provinsi=${row.provinsi}`}>{row.provinsi}</Link></div>);
    }
    ...
    render() {
        ...
        return (
            ...
            namaLaporan = {`Faskes KB Berdasarkan Status Kerjasama Dengan BPJS Kesehatan Dan Klasifikasi KB`}
            marginLeft = {230}
            header = {
                [
                    [
